  ...
  private class Resizer implements Runnable {
    ...
    public void run() {
      try {
        ...
        int subW = Math.min((maxX - minX) + screenshotPaddingWidth * 2, orgImg.getWidth() - subX);
        int subH = Math.min((maxY - minY) + screenshotPaddingHeight * 2, orgImg.getHeight() - subY);
        ...
        LOG.debug("origonal.image.info", new Object[] {orgImg.getWidth(), orgImg.getHeight(), subX,
            subY, subW, subH, file.getAbsolutePath()});
