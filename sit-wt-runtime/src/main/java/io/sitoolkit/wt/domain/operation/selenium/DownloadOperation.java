...
public class DownloadOperation extends SeleniumOperation {
  ...
  public void execute(TestStep testStep, SeleniumOperationContext ctx) {
    ...
    File downloadFile =
        em.buildDownloadFile(current.getScriptName(), current.getCaseNo(), current.getTestStepNo(),
            current.getItemName(), StringUtils.substringAfterLast(urlString, "/"));

    ctx.info(MessagePattern.項目をXXします_URL_エビデンス, MessageManager.getMessage("download"), urlString,
