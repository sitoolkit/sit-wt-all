...
/**
 * WebElementの再実行機能を追加する{@code MethodInterceptor}です。 WebElementの任意のメソッド実行時に例外が発生した場合、 かつその例外が
...
public class WebElementMethodInterceptor implements MethodInterceptor {
  ...
  private static final SitLogger LOG =
      SitLoggerFactory.getLogger(WebElementMethodInterceptor.class);
  ...
  /**
   *
   * @param webDriverFindElementInvocation WebDriver#findElementのMethodInvocationインスタンス
   * @param checker {@code WebElementExceptionChecker}の実装クラスのインスタンス
   */
