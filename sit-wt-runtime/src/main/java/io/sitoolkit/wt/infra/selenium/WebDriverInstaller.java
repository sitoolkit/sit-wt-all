...
public class WebDriverInstaller {
  ...
  public void installSafariDriver() {
    ...
    try {
      ...
      String script =
          IOUtils.toString(ClassLoader.getSystemResource("install-safaridriver.scpt"), "UTF-8");
  ...
  protected synchronized String install(WebDriverBinaryInfo binaryInfo) {
    ...
    try {
      ...
      if (downloadFile.exists()) {
        ...
        LOG.info("download2",
            new Object[] {binaryInfo.sysPropKey, downloadUrl, downloadFile.getAbsolutePath()});
      ...
      if (downloadFile.getName().endsWith(".msi")) {
        ...
        LOG.info("put", new Object[] {binaryInfo.sysPropKey, downloadFile.getAbsolutePath(),
            installFile.getAbsolutePath()});
        FileUtils.copyFile(downloadFile, installFile);

      } else {
        LOG.info("open", new Object[] {binaryInfo.sysPropKey, downloadFile.getAbsolutePath(),
            installFile.getAbsolutePath()});
  ...
  private void extractOneFromTarGz(File srcFile, String entryName, File dstFile)
    ...
    TarArchiveInputStream tarInput =
        new TarArchiveInputStream(new GzipCompressorInputStream(new FileInputStream(srcFile)));
