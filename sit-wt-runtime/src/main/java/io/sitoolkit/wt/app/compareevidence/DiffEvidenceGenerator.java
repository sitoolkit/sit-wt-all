...
public class DiffEvidenceGenerator {
   ...
   * 比較エビデンスのファイルは対象エビデンスディレクトリ内に生成します。 {@code compareScreenshot}に{@code true} を指定すると、
   * エビデンス同士のスクリーンショットを比較し、それに対する比較エビデンスも生成します。
   *
   * @param baseDir 基準エビデンスディレクトリ
   *
   * @param targetDir 対象エビデンスディレクトリ
   *
   * @param compareScreenshot 基準と対象のエビデンスのスクリーンショットの比較を行う場合にtrue
   ...
   * 比較エビデンスを生成します。
   *
   * @param baseEvidenceDir 基準エビデンスディレクトリ
   * @param evidenceFile 比較エビデンスの生成対象のエビデンス
   * @param withUnmatch 不一致スクリーンショットに対する比較エビデンスを生成する場合にtrue
   * @throws IOException 入出力エラーが発生した場合または基準エビデンスが存在しない場合
   */
  ...
  void load(DiffEvidence diffEvidence, EvidenceDir baseEvidenceDir, File baseEvidenceFile,
    ...
    diffEvidence.setLeftFileName(
        FilenameUtils.concat(baseEvidenceDir.getDir().getPath(), baseEvidenceFile.getName()));
    diffEvidence
        .setRightFileName(FilenameUtils.concat(evidenceFile.getParent(), evidenceFile.getName()));
  ...
  void load(DiffEvidence diffEvidence, File baseEvidenceFile, File evidenceFile,
    ...
    diffEvidence
        .setLeftFile(replaceMaskImgName(diffEvidence.getLeftFile(), baseEvidenceFile, withUnmatch));
    diffEvidence
        .setRightFile(replaceMaskImgName(diffEvidence.getRightFile(), evidenceFile, withUnmatch));
  ...
  private String replaceImgName(String text, File evidenceFile) {
    ...
    for (Entry<String, File> imgFile : imgMap.entrySet()) {
      ...
      if (EvidenceDir.isMaskScreenshot(imgName) || EvidenceDir.isUnmatchMaskScreenshot(imgName)) {
        continue;
      }
