...
public class OpenOperationTest {
    ...
    public void testBuildUrlNull() throws IOException {
        String currentDirPath = new File(".").getCanonicalPath().replace("\\", "/") + "/src/main/webapp";
        assertEquals(
                "file:/" + currentDirPath + "/input.html",
                ope.buildUrl(null, "input.html"));
    }
    
    @Test
    public void testBuildUrl() throws IOException {
        String currentDirPath = new File(".").getCanonicalPath().replace("\\", "/");
        
        assertEquals(
                "file:/" + currentDirPath + "/src/main/webapp/input.html",
                ope.buildUrl("src/main/webapp", "input.html"));
    }
}