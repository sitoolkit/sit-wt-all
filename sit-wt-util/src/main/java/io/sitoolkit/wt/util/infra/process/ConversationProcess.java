...
public class ConversationProcess {
  ...
  private static final LogStdoutListener LOG_STDOUT_LISTENER =
      new LogStdoutListener(LOG, Level.INFO, "stdout");
  private static final LogStdoutListener LOG_STDERR_LISTENER =
      new LogStdoutListener(LOG, Level.SEVERE, "stderr");
  ...
  private void wait(List<ProcessExitCallback> callbacks) {
    ...
    try {
      ...
      LOG.log(Level.INFO, "process {0} exits with code : {1}", new Object[] {process, exitCode});
