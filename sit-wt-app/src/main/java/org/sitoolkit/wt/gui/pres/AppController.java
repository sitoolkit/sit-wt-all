...
public class AppController implements Initializable {
	...
	public void initialize(URL location, ResourceBundle resources) {
		...
		if (pomFile == null) {
			messageView.addMsg("[プロジェクト]>[新規作成]からプロジェクトを作成するフォルダを選択してください。");
	...
	public void destroy() {
		...
		if (sampleDir.exists()) {
			conversationProcess.start(new TextAreaConsole(console), sampleDir, MavenUtils.getCommand(), "jetty:stop");
		}
	...
	public void runSample() {
		...
		conversationProcess.onExit(exitCode -> {
			...
			conversationProcess.start(new TextAreaConsole(console, listener), sampledir, MavenUtils.getCommand());

			ExecutorContainer.get().execute(() -> {
				if (listener.isSuccess()) {
					messageView.addMsg("サンプルWebサイトを起動しました。");
					// TODO URLの動的取得
					// baseUrlCombo.setValue("http://localhost:8280");
					testToolbarController.setBaseUrl("http://localhost:8280");
	...
	public void page2script() {
		List<String> command = SitWtRuntimeUtils.buildPage2ScriptCommand(testToolbarController.getDriverType(),
				testToolbarController.getBaseUrl());

		conversationProcess.start(textAreaConsole, projectState.getBaseDir(), command);

		messageView.addMsg("ブラウザでページを表示した状態で「スクリプト生成」ボタンをクリックしてください。");
